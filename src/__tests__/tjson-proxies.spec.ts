...
describe('TJson Proxy Functionality', () => {
  it('should create proxy objects for external non-containment proxies, even if referring object has no ID', () => {
    ...
    expect(deserializedNoId.getManyNonContainment().get(0).eIsProxy()).toBe(
      true
    );
    expect(deserializedNoId.getManyNonContainment().get(0).getId()).toBe(
      referredTo.getId()
    );
  ...
  it('should share same proxy instance for multiple references to same external object', () => {
    ...
    const deserialized = TJson.makeEObject(TJson.makeJson(f1)) as Foo;
    const proxy1 = deserialized
      .getBars()
      .get(0)
      .getBazzles()
      .get(0)
      .getBackupBar();
    const proxy2 = deserialized
      .getBars()
      .get(1)
      .getBazzles()
      .get(0)
  ...
  it('should create proxies for cross-package references', () => {
    ...
    const deserialized = TJson.makeEObject(
      TJson.makeJson(analysisResult)
    ) as AnalysisResult;
  ...
  it('should preserve proxy status through multiple serialization cycles', () => {
    ...
    const secondDeserialized = TJson.makeEObject(
      TJson.makeJson(firstDeserialized)
    ) as Foo;
