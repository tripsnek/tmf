...
export class EcoreParser {
  ...
  public parseFromXmlString(ecoreXml: string) {
    let cartsJs;
    ...
    // console.log(tutils.safeStringify(typesMap));
    ...
    //the root parsed element is an EPackage
    // console.log(
    //   'returning root with subpackages: ' +
    //     rootPackage.getESubPackages().size() +
    //     ' and types: ' +
    //     rootPackage.getEClassifiers().size() +
    //     ' typesMap ' +
    //     typesMap.size
    // );
    // console.error('Beginning Keys');
    // for (const key of typesMap.keys()) {
    //   console.error(key + ' : ' + typeof typesMap[key]);
    // }
    // console.error('Keys over');
    // console.log(tutils.safeStringify(typesMap));
    // .forEach(element => {});
    // typesMap.forEach((value: EClassifier, key: string) => {
    //   console.log(key + ' : ' + typeof value);
    // });
    // console.error('EString type ' + typeof typesMap['#//EString']);
    return rootPackage;
  }
  ...
  private instantiateAllClassifiersAndFeatures(
    ...
    if (pkgJson['eClassifiers']) {
      for (const classEntry of pkgJson['eClassifiers']) {
        ...
        // console.log(
        //   '  - adding ' +
        //     this.getEClassifierUri(path, eclassifier.getName()) +
        //     ' ' +
        //     eclassifier.getName()
        // );
    ...
    if ('eStructuralFeatures' in classEntry) {
      for (const featureEntry of classEntry['eStructuralFeatures']) {
        ...
        if (efeature) {
          ...
          // console.log('adding feature ' + efeature.getName());
