...
export class TGeneratorMain {
  ...
  public async generate(attemptFormatWithPrettier?: boolean): Promise<void> {
    ...
    debug.log('Writing package registry file...');
    await this.writePackageRegistryFile();
  ...
  private async writePackageRegistryFile(): Promise<void> {
    // Only generate for root packages (packages without a super package)
    if (this.pkg.getESuperPackage()) {
      debug.log('Skipping package registry for ' + this.pkg.getName() + ' - not a root package');
      return;
    }

    debug.log('Generating package registry file: package-registry.ts');

    const content = this.generatePackageRegistryContent();
    await this.writeSourceFile(
      this.pkgRootOutDir,
      'package-registry.ts',
      content,
  ...
  private generatePackageRegistryContent(): string {
    return `import { TJson, EClassImpl, EFactory } from '@tripsnek/tmf';
